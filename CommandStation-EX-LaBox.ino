...
hw_timer_t * timerCutOut = NULL;                                         // Ajout RaiCom Timer durée CutOut 
extern int canal;                                                        // Variable n° de canal
void IRAM_ATTR timer_isr_CutOut() {                                      // Iterruption du timer CutOut
  gpio_matrix_out(GPIO_NUM_33, RMT_SIG_OUT0_IDX + canal, 0, 0);          // Reconnecte la pin sur module RMT
}
...
// Set up MotorDrivers early to initialize all pins
  ...
  timerCutOut = timerBegin(3, 80, true);                                   // * Timer 3 
  timerAttachInterrupt(timerCutOut, &timer_isr_CutOut, true);              // * 
  timerAlarmWrite(timerCutOut, 430, false);                                // * 430µs
}

void StarTimerCutOut() {                                                   // *
  timerRestart(timerCutOut);
