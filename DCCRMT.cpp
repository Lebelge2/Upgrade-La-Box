...
void setDCCCBitCutStart(rmt_item32_t* item) {                    // * CutOut Start compris entre 26 et 32µs
  item->level0    = 1;                                           // * Fixé à 30µs    (5+5)x3
  item->duration0 = 5;                                         
  item->level1    = 1;
  item->duration1 = 5;
}
void setDCCBitCutEnd(rmt_item32_t* item) {                       // * Temps CutOut fixé à 424 µs 
  item->level0    = 0;
  item->duration0 = 200;
  item->level1    = 0;
  item->duration1 = 224;
...
int canal;                                                        // * Variable n° de canal
int dp;                                                           // * Variable début préambule
void IRAM_ATTR interrupt(rmt_channel_t channel, void *t) {
 if (!hmi::progMode) {                                           // * Uniquement en mode Main
  canal = channel;                                               // * Mémorise n° de canal
  StarTimerCutOut();                                             // * Active timer CutOut
...
RMTChannel::RMTChannel(pinpair pins, bool isMain) {
  ...
  dp = 0;                                 // * 17 bit de préambule (16 + 1), car le premier est envoyé deux fois (Bug ESP RMT) 
 if (!hmi::progMode) {                                           // * Uniquement en mode Main
  setDCCCBitCutStart(preamble );                                     // * CutOut Start (10µs) envoyé deux fois (Bug ESP RMT) = 20µs
  setDCCCBitCutStart(preamble +1 );                                  // * CutOut Start (10µs)
  setDCCBitCutEnd(preamble + 2);                                     // * CutOut End  (424µs)
  dp = 3;                               // * 17 - 3 = 14 bit de préambule
}
  for (byte n=3; n<plen; n++)                                        // * n = dp.  Début du préambule.
